...
class NGWrap {
  ...
  async deleteStash(index) {
    return nodegit.Stash.pop(this.r, index).catch(normalizeError);
  }
...
const getRepo = async (path) => {
  if (!repoPs[path]) {
    repoPs[path] = nodegit.Repository.open(path)
